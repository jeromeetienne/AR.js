 ...
 */
function averageQuaternion(quaternionSum, newQuaternion, firstQuaternion, count, averageQuaternion){
	averageQuaternion = averageQuaternion || new THREE.Quaternion()
	
	// from http://wiki.unity3d.com/index.php/Averaging_Quaternions_and_Vectors
	if( newQuaternion.dot(firstQuaternion) > 0 ){
		newQuaternion = new THREE.Quaternion(-newQuaternion.x, -newQuaternion.y, -newQuaternion.z, -quaternewQuaternionnion.w)
	}
	...
	averageQuaternion.x = quaternionSum.x/count
	averageQuaternion.y = quaternionSum.y/count
	averageQuaternion.z = quaternionSum.z/count
	averageQuaternion.w = quaternionSum.w/count

	return averageQuaternion
}


function averagePosition(positionSum, position, count, averagePosition){
	averagePosition = averagePosition || new THREE.Vector3()
	
	positionSum.x += position.x
	positionSum.y += position.y
	positionSum.z += position.z
	positionSum.w += position.w
	
	averagePosition.x = positionSum.x / count
	averagePosition.y = positionSum.y / count
	averagePosition.z = positionSum.z / count
	averagePosition.w = positionSum.w / count
...
THREEx.ArMultiMarkerControls.prototype._onSourceProcessed = function(){
	...
	var positionsSum2 = new THREE.Vector3()
	var positionsAverage = new THREE.Vector3()
...
averagePosition(positionsSum2, position, countVisible, positionsAverage)
	...
	if( countVisible > 0 ){
		...
		// var position = _this.object3d.position.clone().lerp(positionsAverage, lerpPosition)
