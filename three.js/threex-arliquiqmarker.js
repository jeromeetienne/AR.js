...
THREEx.ArLiquidMarker = function(videoTexture){
	...
	// var geometry = new THREE.PlaneGeometry(1,1,16,32*2)
	...
	var xMin = -geometry.parameters.width/2
	var xMax =  geometry.parameters.width/2
	var yMin = -geometry.parameters.height/2-0.3
	var yMax =  geometry.parameters.height/2-0.3
	...
	for(var faceIndex = 0; faceIndex < cloakMesh.geometry.faces.length; faceIndex ++ ){
		...
		originalsFaceVertexUvs[0][faceIndex][0] = cloakMesh.geometry.faceVertexUvs[0][faceIndex][0].clone()
		originalsFaceVertexUvs[0][faceIndex][1] = cloakMesh.geometry.faceVertexUvs[0][faceIndex][1].clone()
		originalsFaceVertexUvs[0][faceIndex][2] = cloakMesh.geometry.faceVertexUvs[0][faceIndex][2].clone()
                originalsFaceVertexUvs[0][faceIndex].forEach(function(vector2){
                        vector2.sub(new THREE.Vector2(0.5,0.5))
                        vector2.multiplyScalar(0.5 * cloakMesh.geometry.parameters.width)
	...
	// 	originalsFaceVertexUvs[0][i*4+1][2].set( xMax/2+0.5, yMax/2+0.5 )
        //         
        //         if( i*2+1 >= cloakMesh.geometry.parameters.heightSegments )        break;
        // 
...
THREEx.ArLiquidMarker.markerSpaceShaderFunction = '\n'+
